...
import { ShieldCheck, Zap, AlertTriangle, HelpCircle, FileText, BarChart3, Shield, Info } from 'lucide-react';
...
export default function App() {
  ...
  const sessions = [
    { id: 's1', vehicle: 'Van MH-12-AB-1001', tenant: 'Logistics Fleet A', charger: 'CP-001', soc: 42, targetSoc: 90, allocatedKw: 12.5, state: 'Charging', depTime: '06:00', rank: 1 },
    { id: 's2', vehicle: 'Van MH-12-AB-1002', tenant: 'Logistics Fleet A', charger: 'CP-002', soc: 58, targetSoc: 85, allocatedKw: 11.0, state: 'Charging', depTime: '06:30', rank: 2 },
    { id: 's3', vehicle: 'Truck MH-12-AB-1004', tenant: 'Logistics Fleet A', charger: 'CP-003', soc: 22, targetSoc: 95, allocatedKw: 18.2, state: 'Charging', depTime: '05:45', rank: 1 },
    { id: 's4', vehicle: 'Van MH-12-AB-1003', tenant: 'Logistics Fleet A', charger: 'CP-004', soc: 35, targetSoc: 80, allocatedKw: 0.0, state: 'Paused', depTime: '07:15', rank: 4 },
    { id: 's5', vehicle: 'Express MH-14-XY-2001', tenant: 'Delivery Express B', charger: 'CP-005', soc: 65, targetSoc: 90, allocatedKw: 14.1, state: 'Charging', depTime: '06:15', rank: 1 },
    { id: 's6', vehicle: 'Express MH-14-XY-2002', tenant: 'Delivery Express B', charger: 'CP-006', soc: 71, targetSoc: 85, allocatedKw: 11.0, state: 'Charging', depTime: '06:45', rank: 2 },
  ];
  ...
  const handleWhyClick = (session: any) => {
    setSelectedReceipt({
      session,
      bindingConstraint: session.allocatedKw < 22 ? 'Site Capacity (100 kW Floor)' : 'None',
      shadowPrice: session.allocatedKw < 22 ? 18.50 : 0.0,
      ...
      managerText: session.state === 'Paused'
        ? `Paused to enforce IEC 61851 6A floor without breaching site cap. Rank #${session.rank} in queue.`
        : `Allocated ${session.allocatedKw} kW. Rank #${session.rank} in priority queue for ${session.tenant}.`,
      adminText: `Site capacity binding (λ_site = ₹18.50/kW). Tenant A 40kW floor active. Dual shadow price extracted from GLOP LP relaxation.`
    });
  };
    ...
    <div className="min-h-screen bg-dark-900 text-gray-100 flex flex-col">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-xl font-bold tracking-tight text-white flex items-center gap-2">
              Switchyard <span className="text-xs px-2 py-0.5 rounded bg-blue-500/10 text-blue-400 border border-blue-500/20">v2.0 Plan</span>
            </h1>
      ...
      <main className="flex-1 p-6 space-y-6 max-w-7xl w-full mx-auto">
        ...
        {activeTab === 'LIVE' ? (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="bg-dark-800 p-4 rounded-xl border border-gray-800">
                ...
                <div className="text-2xl font-bold text-white mt-1">6 Vehicles <span className="text-xs text-emerald-400 font-medium">(1 Paused)</span></div>
                  ...
                  <div key={sess.id} className="bg-gray-900/60 p-4 rounded-xl border border-gray-800 flex flex-wrap items-center justify-between gap-4">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-white text-sm">{sess.vehicle}</span>
                        <span className="text-xs px-2 py-0.5 rounded bg-gray-800 text-gray-400">{sess.tenant}</span>
                      </div>
                      <div className="text-xs text-gray-400 flex items-center gap-3">
                        <span>Charger: {sess.charger}</span>
                        <span>Target: {sess.targetSoc}% by {sess.depTime}</span>
                      </div>
                    </div>

                    <div className="flex items-center space-x-6">
                      <div className="text-right">
                        <div className="text-sm font-bold text-blue-400">{sess.allocatedKw} kW</div>
                        <div className="text-xs text-gray-400">SoC: {sess.soc}%</div>
                      </div>
          ...
          /* ACN-Data Benchmark Section (Plan v2 Section 5) */
      ...
      {/* D2 Explainability Modal — Role Tailored Copy */}
      {selectedReceipt && (
            ...
            <div className="space-y-3 text-xs">
              <div className="bg-gray-900 p-4 rounded-xl border border-gray-800 space-y-2">
                <div className="flex justify-between text-gray-300">
                  <span>Energy Consumed (142.5 kWh @ ₹12/kWh):</span>
                  <span className="font-semibold text-white">₹1,710.00</span>
                </div>
                <div className="flex justify-between text-gray-300">
                ...
                <div className="border-t border-gray-800 pt-2 flex justify-between text-sm font-bold text-white">
                  ...
                  <span className="text-blue-400">₹2,065.00</span>
                </div>
              </div>
            </div>
